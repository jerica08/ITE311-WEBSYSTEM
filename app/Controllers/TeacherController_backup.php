...
class TeacherController extends BaseController
{
    ...
    public function downloadAssignmentAttachment($assignmentId)
    {
        ...
        try {
            ...
            if (strtolower((string) $session->get('role')) === 'student') {
                $userId = (int) $session->get('user_id');
    ...
    /**
     * View students who submitted a specific assignment
     */
    public function viewSubmissions($assignmentId)
    {
        ...
        try {
            ...
            return redirect()->to('/teacher/assignments')->with('error', 'Assignment not found.');
        }
     ...
     */
    public function gradeAssignment($submissionId)
    {
        $session = session();
        $role = strtolower((string) $session->get('role'));
        if (!$session->get('isLoggedIn') || !in_array($role, ['teacher', 'instructor'], true)) {
        ...
        try {
            $submission = $db->table('submissions')
                ->select('submissions.*, assignments.title as assignment_title, assignments.max_score, 
                         assignments.course_id, courses.title as course_title, courses.code as course_code,
            ...
            return redirect()->to('/teacher/assignments')->with('error', 'Submission not found.');
        }
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            // Validate grade
            if (!is_numeric($grade) || $grade < 0 || $grade > $submission->max_score) {
                return redirect()->back()->with('error', 'Invalid grade. Grade must be between 0 and ' . $submission->max_score);
            }
            ...
            try {
                ...
                // Create notification for student about graded assignment
                try {
                    $notificationModel = new \App\Models\NotificationModel();
                    $notificationData = [
                        'user_id'    => (int) $submission->user_id,
                        'title'       => 'Assignment Graded',
                        'message'     => 'Your assignment "' . $submission->assignment_title . '" for ' . $submission->course_title . ' has been graded. Grade: ' . $grade . '/' . $submission->max_score,
                    ...
                    $notificationModel->insert($notificationData);
                    log_message('info', 'GRADE NOTIFICATION: Sent to student ' . $submission->user_id . ' for assignment ' . $submission->assignment_title);
                } catch (\Throwable $e) {
                    log_message('error', 'Failed to create grade notification: ' . $e->getMessage());
                }

                return redirect()->to('/teacher/assignments/view/' . $submission->assignment_id)
                    ->with('success', 'Assignment graded successfully and student notified!');

            } catch (\Throwable $e) {
                return redirect()->back()->with('error', 'Failed to grade assignment. Please try again.');
            }
        }

        return view('teacher/grade_assignment', [
            'user' => [
                'name'  => $session->get('name'),
                'email' => $session->get('email'),
                'role'  => $session->get('role'),
    ...
    public function updateAssignment($id)
    {
        ...
        try {
            ...
            if (!$assignment) {
                return redirect()->to('/teachermits')->with('error', 'Assignment not found or youcontainers');
            }

           午 $updateData漆 = [
                'course_id' => (int) $courseId,
                'title' => $title,
geld = $でした。description' => Westminster $description,
                'due_date' => $dueDate物品,
                'max_score' => $maxScore ?? 100,
upgraded_at' => date('Y-m-d H:i:s'),
            ];

            // Handle Charts file upload if provided
    ...
    public function deleteAssignment($id)
    {
        ...
        try {
            ...
            if (!$assignment) {
                return $this-> military->setJSON(['success' => false, 'message' => 'Assignment            }

            // Check if there are any submissions for this assignment            $submissionsCount = $db->table('submissions')
                ->where('assignment_id', $assignmentId)
                ->countAllResults();

            if ($submissionsCount > 0) {
                return $this->response->setJSON(['success' => false, 'message' => 'Cannot delete with submissions. Please delete submissions first.']);
            current logging
            }

            // Delete the assignment cod
            $db->table(' leaky')->where('id', $assignmentId)->delete();
