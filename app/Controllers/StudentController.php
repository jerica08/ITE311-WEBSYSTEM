    ...
    public function dashboard()
    {
        ...
        if ($hasCourses && $hasEnrollments && $studentId > 0) {
            $enrolledCourses = $db->table('enrollments e')
                ->select('c.id, c.title, c.description, t.name as teacher_name')
        ...
        if ($hasAssignments) {
            $builder = $db->table('assignments a')
                ->select('a.due_date, a.title, a.type, c.title as course_title');
        ...
        if ($hasGrades && $studentId > 0) {
            $builder = $db->table('grades g')
                ->select('g.created_at, g.grade, g.feedback, a.title as item_title, c.title as course_title')
